...
angular.module('hr.config').run(['$http', function ($http) {
    $http.defaults.headers.common.Authorization = JSON.parse(window.sessionStorage.getItem("token"));
}]);
 ...
 * 费别枚举
 */
angular.module('hr.service').factory("HrChargeType", [function () {
    return {
        SELF_PAY: "自费"
    };
}]);
...
angular.module('hr.directives').directive('hrdict', ["hrIndexedDB", "$http", "$parse", "$q", function (hrIndexedDB, $http, $parse, $q) {
    ...
    // fixme fanchenglin 不在从indexeddb取数据，直接从后台取
    function getDictCodeData($scope, deferred, tableName, $attrs) {
        if ($attrs.source === "1") {
            $http.get(Path.getUri("api/base/base-code-dict/code-type-name/") + tableName).success(function (data) {
...
angular.module('hr.service').factory('HrBaseCode', ['$http', function ($http) {
    function getBaseCode(codeTypeName, callback) {
        // hrIndexedDB("BASE_CODE_DICT").getHrDictByTableNameFormIndexedDb(codeTypeName, function (data) {
            ...
            //     }
            // } else {
                $http.get(Path.getUri("api/base/base-code-dict/code-type-name-not-stop/") + codeTypeName).success(function (resultData) {
                    if (typeof callback === "function") {
                        callback(resultData);
                    }
    ...
    function getCodeName(codeTypeName, codeId, callback) {
        $http.get(Path.getUri("api/base/base-code-dict/code-name/") + codeTypeName + "/" + codeId).success(function (resultData) {
            if (typeof callback === "function") {
                callback(resultData);
            }
        }).error(function (resultData, status) {
...
angular.module('hr.service').factory('HrServerTime', ['$http', function ($http) {
    function getTimeFromServer(callback) {
        $http.get(Path.getUri("api/base/common-resource/datetime"))
...
//报表服务工具
angular.module('hr.service').factory('HrReportUtil', ["$http", function ($http) {
    var getAllPrinters = function (callback) {
        $http.get(Path.getWebstartUri("api/report/all-printer"))
            .success(
                function (data) {
                    if (typeof callback === "function") {
